...
import {Button, Checkbox, Divider, Form, message, Input, notification, InputNumber, Modal, Select} from "antd";
...
export default function AddItem() {
    ...
    * */
    ...
    function addItemToDB(data) {
        ...
        dbService
            ...
            .then(() => {
                // reset form
                form.resetFields();
                notification.success({message: "The item was added", placement: "topRight"});
                // clear images from wall
                setClearImagesStatus(true);
            }).catch(err => {
            console.error(err);
            notification.error({message: "Opps! Unable to add this item", description: err.message});
    ...
    return(
        <>
            <Head>
                <title>Add Item to Inventory - BICHE ADMIN</title>
            </Head>
            <AdminNav>
                <section className="admin-content px-10 pt-6">
                    <div className="row h-full">
                                            ...
                                            >
                                                {
                                                    categories.map((cat, index) => {
                                                        return <Option key={"cat-op-" + index} value={cat.name}>{cat.name}</Option>
                                                    })
                                                }
                                            </Select>
                                        </Form.Item>
                        ...
                        <div className="col-sm-5">
                            ...
                            <ImageUploadWall max={4} onUpload={onFileUpload} clearAll={clearImagesStatus}/>
                        </div>
                    </div>
                </section>
            </AdminNav>
